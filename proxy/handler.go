...
type RequestLog struct {
	...
	InputTokens      int     `json:"inputTokens"`      // Visible, uncached input tokens
	CacheTokens      int     `json:"cacheTokens"`      // Cache read + cache write tokens
	CacheReadTokens  int     `json:"cacheReadTokens"`  // Tokens read from prompt cache
	CacheWriteTokens int     `json:"cacheWriteTokens"` // Tokens written to prompt cache
	OutputTokens     int     `json:"outputTokens"`     // Output tokens
	Tokens           int     `json:"tokens"`           // Total tokens (input+cache+output)
...
func (h *Handler) handleClaudeStream(w http.ResponseWriter, payload *KiroPayload, model string, thinking bool, thinkingOpts claudeThinkingResponseOptions, estimatedInputTokens, billingInputTokens int, contextDebug *contextDebugTask, cacheProfile *promptCacheProfile, apiKeyID string, usageReportWindow int) {
	...
	for attempt := 0; attempt < maxAccountRetryAttempts; attempt++ {
		...
		cacheNamespace := account.ID
		cacheUsage := h.promptCache.Compute(cacheNamespace, cacheProfile)
		logger.Debugf("[ClaudeUsage] stream namespace=%q account=%s cache_usage={creation:%d read:%d covered_est:%d prompt_total_est:%d}",
			cacheNamespace, account.ID, cacheUsage.CacheCreationInputTokens, cacheUsage.CacheReadInputTokens, cacheUsage.CacheCoveredEstimate, cacheUsage.PromptTotalEstimate)
		...
		h.recordSuccessLog("claude", model, account.ID, visibleInputTokens,
			publicCacheUsage.CacheReadInputTokens, publicCacheUsage.CacheCreationInputTokens,
			outputTokens, credits, time.Since(reqStart).Milliseconds())
...
// recordSuccessLog records a successful request in the request logs. Tokens
// use the public usage breakdown so total always equals
// input+cache_read+cache_write+output.
func (h *Handler) recordSuccessLog(endpoint, model, accountID string, inputTokens, cacheReadTokens, cacheWriteTokens, outputTokens int, credits float64, durationMs int64) {
	...
	h.appendRequestLog(entry)
	logger.Infof("[TokenUsage] endpoint=%s model=%s account=%s input=%d cache_read=%d cache_write=%d output=%d total=%d",
		endpoint, model, accountID, inputTokens, cacheReadTokens, cacheWriteTokens, outputTokens, totalTokens)
}
...
func (h *Handler) handleClaudeNonStream(w http.ResponseWriter, payload *KiroPayload, model string, thinking bool, thinkingOpts claudeThinkingResponseOptions, estimatedInputTokens, billingInputTokens int, contextDebug *contextDebugTask, cacheProfile *promptCacheProfile, apiKeyID string, usageReportWindow int) {
	...
	for attempt := 0; attempt < maxAccountRetryAttempts; attempt++ {
		...
		cacheNamespace := account.ID
		cacheUsage := h.promptCache.Compute(cacheNamespace, cacheProfile)
		logger.Debugf("[ClaudeUsage] nonstream namespace=%q account=%s cache_usage={creation:%d read:%d covered_est:%d prompt_total_est:%d}",
			cacheNamespace, account.ID, cacheUsage.CacheCreationInputTokens, cacheUsage.CacheReadInputTokens, cacheUsage.CacheCoveredEstimate, cacheUsage.PromptTotalEstimate)
		...
		if cacheProfile != nil {
			resp.Usage.CacheCreation = &ClaudeCacheCreationUsage{
				Ephemeral5mInputTokens: publicCacheUsage.CacheCreation5mInputTokens,
...
func (h *Handler) handleOpenAIStream(w http.ResponseWriter, payload *KiroPayload, model string, thinking bool, estimatedInputTokens int, apiKeyID string, usageReportWindow int) {
	...
	for attempt := 0; attempt < maxAccountRetryAttempts; attempt++ {
		...
		h.pool.RecordSuccess(account.ID)
		h.pool.UpdateStats(account.ID, inputTokens+outputTokens, credits)
		h.recordSuccessLog("openai", model, account.ID, inputTokens, 0, 0, outputTokens, credits, time.Since(reqStart).Milliseconds())
